...
export function TaskForm({ now, task, user, locale, t, save, close }: { now: number; task?: Task; user: User; locale: Locale; t: Translate; save: (body: Record<string, unknown>, id?: string) => Promise<void>; close: () => void }) {
  ...
  return <Modal title={task ? t('Chỉnh sửa công việc', 'Edit task') : t('Thêm việc cần làm', 'Create a task')} subtitle={t('Nói bạn cần làm gì. Chúng mình sẽ gợi ý lúc phù hợp.', "Tell us what you need to do. We'll help find the time.")} onClose={close} busy={busy}>
    <form onSubmit={submit} className="form-stack">{error && <ErrorNotice>{error}</ErrorNotice>}
      ...
      <Field label={t('Hoàn thành trước', 'Finish before')} hint={user.timezone}><input type="datetime-local" name="deadline" required defaultValue={task ? toInput(task.deadline, user.timezone) : `${shiftDay(localDay(user.timezone, now), 2)}T22:00`} /></Field>
...
export function SuggestionDetails({ now, suggestion, user, locale, t, decide, close }: { now: number; suggestion: Suggestion; user: User; locale: Locale; t: Translate; decide: (id: string, action: 'accept' | 'reject') => Promise<void>; close: () => void }) {
